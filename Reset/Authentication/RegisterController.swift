...
class RegisterController: UIViewController {
    ...
    @objc func didTapSignUp() {
        ...
        AuthService.shared.registerUser(with: registerUserRequest) { [weak self] wasRegistered, error in
            ...
            if wasRegistered {
                // Navigate to SpendViewController instead of checking authentication
                DispatchQueue.main.async {
                    let spendVC = SpendViewController()
                    
                    // If we're in a navigation controller, push the SpendViewController
                    if let navigationController = self.navigationController {
                        navigationController.pushViewController(spendVC, animated: true)
                    } else {
                        // If we're not in a navigation controller, present it modally
